...
class RateLimitError(Exception):
    ...
    def __init__(self, retry_after: datetime.datetime) -> None:
        """Create error."""
        self.message = "The hypixel API ratelimit was reached, " + f"try again at {retry_after.strftime('%H:%M:%S')}."
...
class ApiNoSuccessError(Exception):
    ...
    def __init__(
        ...
        """Create error."""
        self.message = f"The {source} endpoint encountered an error on the hypixel side."
        super().__init__(self.message)
