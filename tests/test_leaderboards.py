...
async def test_leaderboards(
    ...
    with aioresponses() as m:
        m.get(
            f"https://api.hypixel.net/leaderboards?key={str(key)}",
            status=200,
            headers={
                "RateLimit-Limit": "120",
        ...
        assert data["SKYWARS"][0].leaders[0] == uuid.UUID(
            "2afdb69c-c007-40cd-98b9-76a7554612d9"
        )
        assert data["SKYWARS"][0].leaders[1] == uuid.UUID(
            "6951ccdb-9ca7-4c8a-883b-a8d3bb81c3d2"
        )
        assert data["SKYWARS"][0].leaders[2] == uuid.UUID(
            "e61044cc-c42f-439b-9ad7-817c51ae7174"
        )
